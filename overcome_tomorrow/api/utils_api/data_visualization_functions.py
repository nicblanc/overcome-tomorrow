...
def plot_monthly_distance_over_years(activity_df):
    ...
    monthly_distance = activity_df.groupby(['year', 'month'])[
        'total_distance'].sum()
...
def plot_monthly_distance(year, month, activity_df):
    ...
    monthly_data = activity_df[(activity_df['start_time'].dt.year == year) & (
        activity_df['start_time'].dt.month == month)]

    # Aggregate the data on total distance covered for each day
    aggregated_data = monthly_data.groupby(monthly_data['start_time'].dt.day)[
        'total_distance'].sum()

    # Plot the total distance covered for each day of the specified month and year
    plt.figure(figsize=(10, 6))
    plt.plot(aggregated_data.index,
             aggregated_data.values, marker='o', color='b')
    plt.title(
        f'Total Distance Covered in {pd.Timestamp(year=year, month=month, day=1).strftime("%B %Y")}')
    plt.xlabel('Day')
    plt.ylabel('Total Distance (meters)')
    plt.grid(True)
    plt.tight_layout()
...
def plot_monthly_calorie_and_distance(activity_df):
    ...
    ax2 = ax1.twinx()
    ax2.plot(months, activity_df.groupby('month')[
             'total_distance_km'].sum(), marker='o', color='orange')
    ax2.set_ylabel('Total Distance (km)', color='orange')
    ax2.tick_params(axis='y', labelcolor='orange')

    # Adding legend and title
    plt.title('Monthly Calorie Expenditure and Distance Covered')
    plt.xticks(months, ['Jan', 'Feb', 'Mar', 'Apr', 'May',
...
def plot_average_speed_over_time(activity_df):
    ...
    activity_df_copy = activity_df_copy[activity_df_copy['enhanced_avg_speed'] > 0]

    # Convert the 'timestamp' column to datetime format if not already done
    activity_df_copy['timestamp'] = pd.to_datetime(
        activity_df_copy['timestamp'])

    activity_df_copy['enhanced_avg_speed'] = activity_df_copy['enhanced_avg_speed'] * 3.6
    activity_df_copy.set_index("timestamp", inplace=True)

    activity_df_copy = activity_df_copy[["enhanced_avg_speed", "sport"]]

    # Sort the data by timestamp
    activity_df_copy.sort_index(inplace=True)

    # Create the line plot
    plt.figure(figsize=(10, 6))
    # round(json_response.get('enhanced_max_speed')*3.6, 2)

    runnning_df = activity_df_copy[activity_df_copy["sport"] == "running"]
    cycling_df = activity_df_copy[activity_df_copy["sport"] == "cycling"]
    swimming_df = activity_df_copy[activity_df_copy["sport"] == "swimming"]

    runnning_df = runnning_df.resample('MS').mean()
    cycling_df = cycling_df.resample('MS').mean()
    swimming_df = swimming_df.resample('MS').mean()

    plt.plot(runnning_df.index,
             runnning_df['enhanced_avg_speed'].round(2), color='blue', label="Running")
    plt.plot(cycling_df.index,
             cycling_df['enhanced_avg_speed'].round(2), color='red', label="Cycling")
    plt.plot(swimming_df.index,
             swimming_df['enhanced_avg_speed'].round(2), color='green', label="Swimming")

    # Add labels and title
    plt.title('Evolution of Average Speed over Time (km/h)')
    plt.xlabel('Date')
    plt.ylabel('Average Speed')
    plt.legend()
    plt.grid(True)
...
def analyze_last_activities(activity_df, num_days, sport_type):
    ...
    last_activities = activity_df[activity_df['sport']
                                  == sport_type].tail(num_days)
    ...
    plt.figure(figsize=(10, 6))
    sns.barplot(data=last_activities, x=days_range,
                y='total_distance_km', color='skyblue', label='Total Distance')
    ax2 = plt.twinx()
    sns.lineplot(data=last_activities, x=days_range, y='avg_heart_rate',
                 marker='o', color='orange', ax=ax2, label='Average Heart Rate')

    # Adding labels and title
    plt.title(
        f'Analysis of Last {num_days} Days of {sport_type.capitalize()} Activities')
    plt.xlabel('Days')
