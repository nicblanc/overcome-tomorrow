...
def predict_next_activity(models: str = "DEFAULT"):
    # TODO get activity for each model
    return [predict_next_activity(DEFAULT_ACTIVITY) for model in models.split(",")]


@tomorrow_app.get("/activities/next/{model}", tags=["activities"])
def predict_next_activity(model: str):
